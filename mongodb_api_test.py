...
def create_test_room_in_mongodb():
...
def test_room_via_api(room_id):
    """Test a room via the API"""
...
def run_room_lifecycle_tests():
    ...
    # Create a test room directly in MongoDB
    room_id = create_test_room_in_mongodb()
    
    # Test the room via the API
    print("\n" + "="*80)
    print("Test: Room Lifecycle via API")
    print("="*80)
    room_lifecycle_result = test_room_via_api(room_id)
    
    # Print summary
    print(f"\n{'='*80}\nTest Summary\n{'='*80}")
    print(f"Room Lifecycle via API: {'PASSED' if room_lifecycle_result else 'FAILED'}")
    
    return room_lifecycle_result
